...
import http from './http';
  ...
  const checkSmsCode = (params) => {
    ...
    return  http.post("/wp-content/plugins/user-action-plugin/wp-remote-check-sms-code.php", params);
  };
    //用户登录
    ...
    return  http.post("/wp-content/plugins/user-action-plugin/wp-remote-login.php", params);
  };

  const userRegister = (params) => {
    // return  axios.post(`${constant.baseUrl}/login/reg`, params);
    return  http.post("/wp-content/plugins/user-action-plugin/wp-remote-register.php", params);
  };
